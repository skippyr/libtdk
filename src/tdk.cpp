...
/**
 * @brief Checks the status a standard terminal stream being targeted and cause
 * an early return if it is not a TTY.
 */
...
/**
 * @brief Caches information about TTY statuses of the standard terminal streams
 * and sets the ENABLE_VIRTUAL_TERMINAL_PROCESSING flag on Windows.
 */
  ...
  writeANSISequence(
      ...
      static_cast<int>(
          static_cast<int>(shape) >=
                      static_cast<int>(tdk::CursorShape::Default) &&
                  static_cast<int>(shape) <=
