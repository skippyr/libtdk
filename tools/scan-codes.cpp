...
#define BOOLEAN_STATE(a_value)                                                 \
  (a_value ? "\x1b[32menabled\x1b[39m" : "\x1b[31mdisabled\x1b[39m")

static void writeHelpPage() {
  std::cout
      << "\x1b[1mUsage:\x1b[22m \x1b[32mscan-codes "
         "\x1b[39m[\x1b[33mOPTION\x1b[39m]..."
      << std::endl
      << "Writes the scan codes related to key, focus and mouse events."
      << std::endl
      << "By default, it only handle key events." << std::endl
      << std::endl
      << "\x1b[1mOPTIONS\x1b[22m" << std::endl
      << "           \x1b[33m--help\x1b[39m  shows these help instructions."
      << std::endl
...
int main(int totalArguments, const char **arguments) {
  ...
  for (int offset = 0; offset < totalArguments; ++offset) {
    if (!std::strcmp(arguments[offset], "--allow-focus")) {
      ...
      std::cerr
          << "\x1b[1;31m[ERROR]\x1b[22;39m Use of unrecognized flag \x1b[33m"
          << arguments[offset] << "\x1b[39m." << std::endl
          << "        For more info about options, use the "
             "\x1b[33m--help\x1b[39m flag."
          << std::endl;
      return 1;
    }
  }
  ...
  std::cout << "\x1b[?25lFor more info, use the \x1b[33m--help\x1b[39m option."
            << std::endl
            << "Press the [\x1b[32mEscape\x1b[39m] key to exit" << std::endl
            << std::endl;
  std::cout << "    ~> Focus events are " << BOOLEAN_STATE(allowFocus) << "."
            << std::endl
            << "    ~> Mouse events are " << BOOLEAN_STATE(allowMouse) << "."
            << std::endl
            << std::endl;
  ...
  while (true) {
    ...
    for (offset = 0; (character = std::getchar()) != EOF; ++offset) {
      std::cout << " "
                << (character == 59 ? "\x1b[33m" + std::to_string(character) + "\x1b[39m"
